...
dat <- fread("data_Maniscalco_2017_expt1.csv", header = T)
dat <- filter(dat, Subj_idx != 3 & Subj_idx != 6 & Subj_idx != 9 & Subj_idx != 22) 
...
df %>%
  ...
  theme(axis.title = element_text(size = 7), legend.position = "top", plot.title = element_text(size = 7),
        ...
        plot.margin = unit(c(1, 1, 1, 1), "pt")) + ggtitle("Maniscalco_2017_expt1") -> p1
...
df %>%
  ...
  theme(axis.title = element_text(size = 7), legend.position = "top", plot.title = element_text(size = 7),
        strip.text = element_text(size = 7, margin = margin(-0.5, -0.5, -0.5, -0.5, "pt")),
        plot.margin = unit(c(1, 1, 1, 1), "pt")) + ggtitle("Maniscalco_2017_expt1") -> p2
