...
df <- foreach(i = 1:length(unique(dat$Subj_idx)), .combine = "rbind", .packages = c("dplyr", "metaSDT")) %dopar% {
  ...
  d <- mutate(d, Confidence = ifelse(Confidence < 3, 1,
                                     ifelse(Confidence < 5, 2, 3)))
  # RT bin
...
ggplot(df1) + 
  ...
  xlab("Confidence") + ylab("Accuracy") + coord_cartesian(ylim = c(0.45, 1)) +
  theme(axis.title = element_text(size = 7), legend.position = "top", plot.title = element_text(size = 7),
        ...
        plot.margin = unit(c(1, 1, 1, 1), "pt")) + ggtitle("Reyes_2015") -> p1
...
ggplot(df2) + 
  ...
  theme(axis.title = element_text(size = 7), legend.position = "top", plot.title = element_text(size = 7),
        strip.text = element_text(size = 7, margin = margin(-0.5, -0.5, -0.5, -0.5, "pt")),
        plot.margin = unit(c(1, 1, 1, 1), "pt")) + ggtitle("Reyes_2015") -> p2
