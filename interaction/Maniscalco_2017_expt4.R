...
dat <- fread("data_Maniscalco_2017_expt4.csv", header = T)
dat <- filter(dat, Subj_idx != 3 & Subj_idx != 4 & Subj_idx != 14 & 
              Subj_idx != 25 & Subj_idx != 26 & Subj_idx != 32) 
...
df %>%
  ...
  theme(axis.title = element_text(size = 7), legend.position = "top", plot.title = element_text(size = 7),
        ...
        plot.margin = unit(c(1, 1, 1, 1), "pt")) + ggtitle("Maniscalco_2017_expt4") -> p1
...
df %>%
  ...
  theme(axis.title = element_text(size = 7), legend.position = "top", plot.title = element_text(size = 7),
        strip.text = element_text(size = 7, margin = margin(-0.5, -0.5, -0.5, -0.5, "pt")),
        plot.margin = unit(c(1, 1, 1, 1), "pt")) + ggtitle("Maniscalco_2017_expt4") -> p2

df1 %>%
  arrange(RT_bin, Confidence) %>%
  group_by(RT_bin) %>%
  mutate(slope = lead(mean_accuracy, 1) - mean_accuracy,
         pair = rep(c("2-1", "3-2", "4-3", "NA"))) -> slope_1
  
df2 %>%
  arrange(RT_bin, Confidence) %>%
  group_by(RT_bin) %>%
  mutate(slope = lead(mean_logit, 1) - mean_logit,
         pair = rep(c("2-1", "3-2", "4-3", "NA"))) -> slope_2