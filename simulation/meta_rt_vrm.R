  ...
  for (j in eta) {
    for (k in sz) {
      ...
      d <- foreach(l = 1:trial, .combine = "rbind", .packages = c("dplyr", "matrixStats")) %dopar% {
        ...
        simrt  <- which(matrixStats::rowMaxs(accum) > a)[1]
...
simdat_vrm <- rbind(df1, df2)
simdat_vrm$choice <- factor(simdat_vrm$choice, levels = c("target", "distractor")) # this is required
simdat_vrm$correct = ifelse(simdat_vrm$choice == "target", 1, 0)
simdat_vrm <- na.omit(simdat_vrm)
simdat_vrm <- mutate(simdat_vrm, early_conf = mvn_350 - mvn_0)
simdat_vrm <- mutate(simdat_vrm, post_conf  = e_chosen_post - e_second_post - mvn_response)
...
for (i in nu_tar) {
  for (j in eta) {
    for (k in sz) {
      df <- subset(simdat_vrm, simdat_vrm$nu_tar == i & simdat_vrm$eta == j & simdat_vrm$sz == k)
...
roc_1_vrm <- c()
roc_2_vrm <- c()
for (i in unique(simdat_vrm$nu_tar)) {
  for (j in unique(simdat_vrm$eta)) {
    for (k in unique(simdat_vrm$sz)) {
      
      sd <- subset(simdat_vrm, simdat_vrm$nu_tar == i & simdat_vrm$eta == j & simdat_vrm$sz == k)
      ...
      dfc_1$variable <- "post"
      sdt_vrm_c <- fit_meta_d_MLE(h1c, f1c)
      ...
      dfb_1$variable <- "be"
      sdt_vrm_b <- fit_meta_d_MLE(h1b, f1b)
      ...
      dfr_1$variable <- "RT"
      sdt_vrm_r <- fit_meta_d_MLE(h1r, f1r)
      ...
      sdt_vrm_lb <- fit_meta_d_MLE(h1lb, f1lb)
      ...
      sdt_vrm_lp <- fit_meta_d_MLE(h1lp, f1lp)
      ...
      sdt_vrm <- rbind(sdt_vrm, cbind(sdt_vrm_b[1, 1], sdt_vrm_b[1, 3], sdt_vrm_c[1, 3], sdt_vrm_r[1, 3], sdt_vrm_lb[1, 3], sdt_vrm_lp[1, 3], i, j, k))
      roc_1_vrm <- rbind(roc_1_vrm, df_1)
      roc_2_vrm <- rbind(roc_2_vrm, df_2)
    }
  }
}

# sdt
colnames(sdt_vrm) <- c("dp", "mdp_be", "mdp_post", "mdp_rt", "mdp_logit_be", "mdp_logit_post", "nu_tar", "eta", "sz")
sdt_vrm <- as.data.frame(sdt_vrm)
sdt_vrm <- mutate(sdt_vrm, 
              mratio_be = mdp_be / dp,
...
for (l in 1:nrow(sdt_vrm)) {
  ...
  roc_1_sdt_vrm <- rbind(roc_1_sdt_vrm, data.frame(hr_1 = cumsum(resp2)/sum(resp2), far_1 = cumsum(resp1)/sum(resp1), 
                                           nu_tar = sdt_vrm$nu_tar[l], eta = sdt_vrm$eta[l], sz = sdt_vrm$sz[l]))
  roc_2_sdt_vrm <- rbind(roc_2_sdt_vrm, data.frame(hr_2 = cumsum(h2)/sum(h2), far_2 = cumsum(f2)/sum(f2),
                                           nu_tar = sdt_vrm$nu_tar[l], eta = sdt_vrm$eta[l], sz = sdt_vrm$sz[l]))
}
...
simdat_vrm %>%
  ...
  ggplot() + geom_line(aes(x = nu_tar, y = p, color = factor(choice)), size = 0.3) + 
  geom_point(aes(x = nu_tar, y = p, color = choice), size = 0.3) + 
  ...
  scale_color_npg() + coord_cartesian(ylim = c(0, 1)) + ggtitle("Race model") -> p1_vrm
p1_vrm
p1_vrm <- p1_vrm + guides(color = F) + theme(legend.text = element_text(size = 3),
                                             legend.position = "top", 
                                             legend.direction = "horizontal",
                                             legend.background = element_rect(fill = NA, colour = NA),
                                             legend.key = element_rect(fill = NA),
...
simdat_vrm %>%
  ...
  scale_fill_npg() + scale_color_npg() + ggtitle("Race model") +
  theme(axis.title = element_text(size = 7), legend.position = "top", plot.title = element_text(size = 7),
        strip.text = element_text(size = 7, margin = margin(-0.5, -0.5, -0.5, -0.5, "pt"))) -> p2_vrm
p2_vrm
p2_vrm <- p2_vrm + guides(color = F, fill = F) +
...
simdat_vrm %>%
  ...
  scale_x_continuous(breaks = c(0.0015, 0.0025, 0.0035)) + xlab(expression(bold(paste({ν[target]})))) +
  facet_wrap(~ condition, labeller = label_parsed, nrow = 2)  +
  ylab("Mean RT (s)") + scale_color_npg() + coord_cartesian(ylim = c(0.5, 1.5)) + ggtitle("Race model") -> p3_vrm
p3_vrm
p3_vrm <- p3_vrm + guides(color = F) + theme(legend.text = element_text(size = 3),
                                             legend.position = "top", 
                                             legend.direction = "horizontal",
                                             legend.background = element_rect(fill = NA, colour = NA),
                                             legend.key = element_rect(fill = NA),
...
simdat_vrm %>%
  group_by(choice, nu_tar, eta, sz) %>%
  mutate(mean_conf = mean(mvn_response)) %>%
  mutate(condition = ifelse(eta == 5e-04 & sz == 0.1, "η == 0.0005 ~ phantom(',') ~ s[z] == 0.1",
                            ifelse(eta == 5e-04 & sz == 0.6, "η == 0.0005 ~ phantom(',') ~ s[z] == 0.6",
                                   ifelse(eta == 0.005 & sz == 0.1, "η == 0.005 ~ phantom(',') ~ s[z] == 0.1",
                                          "η == 0.005 ~ phantom(',') ~ s[z] == 0.6")))) %>%
  ...
  scale_x_continuous(breaks = c(0.0015, 0.0025, 0.0035)) + xlab(expression(bold(paste({ν[target]})))) +
  facet_wrap(~ condition, labeller = label_parsed, nrow = 2)  +
  ylab("Mean confidence") + scale_color_npg() + ggtitle("Race model") -> p4_vrm
p4_vrm
p4_vrm <- p4_vrm + guides(color = F) + theme(legend.text = element_text(size = 3),
                                             legend.position = "top", 
                                             legend.direction = "horizontal",
                                             legend.background = element_rect(fill = NA, colour = NA),
                                             legend.key = element_rect(fill = NA),
...
simdat_vrm %>%
  ...
  geom_point(aes(x = nu_tar, y = mean_conf, color = factor(choice)), size = 0.7) +
  scale_color_discrete(labels = c("target", "distractor")) + labs(color = "choice") +
  scale_x_continuous(breaks = c(0.0015, 0.0025, 0.0035)) + xlab(expression(bold(paste({ν[target]})))) +
  facet_wrap(~ condition, labeller = label_parsed, nrow = 2)  +
  ylab("Mean post confidence") + scale_color_npg() + coord_cartesian(ylim = c(-0.5, 1.2)) + ggtitle("Race model") -> p5_vrm
p5_vrm
p5_vrm <- p5_vrm + guides(color = F) + theme(legend.text = element_text(size = 3),
                                             legend.position = "top", 
                                             legend.direction = "horizontal",
                                             legend.background = element_rect(fill = NA, colour = NA),
                                             legend.key = element_rect(fill = NA),
...
# sdt measures (BE)
sdt_vrm %>%
  pivot_longer(cols = c("dp", "mdp_be", "mdp_logit_be", "mdp_rt", ), 
               names_to = "index", values_to = "value") %>%
  mutate(index = fct_recode(index, "d'" = "dp", "meta-d' confidence" = "mdp_be", 
                            "meta-d' RT" = "mdp_rt", "meta-d' logit" = "mdp_logit_be"),
  ...
  scale_color_manual(values = c("black", hue_pal()(3))) + ylab("meta-d'") +
  scale_x_continuous(breaks = c(0.0015, 0.0025, 0.0035)) + xlab(expression(bold(paste({ν[target]})))) +
  facet_wrap(~ condition, labeller = label_parsed, nrow = 2, scales = "free_y") + ggtitle("Race model") -> p6_vrm
p6_vrm
...
p6_vrm <- p6_vrm + facetted_pos_scales(y = position_scales) + 
  guides(color = F) +
  theme(strip.text = element_text(size = 4.2, face = "bold", color = "black"),
        plot.margin = margin(5, 5, 5, 5), 
        plot.title = element_text(size = 5), 
        axis.title.x = element_text(size = 5),
        axis.title.y = element_text(size = 5),
...
sdt_vrm %>%
  ...
  mutate(condition = ifelse(eta == 5e-04 & sz == 0.1, "η == 0.0005 ~ phantom(',') ~ s[z] == 0.1",
                            ifelse(eta == 5e-04 & sz == 0.6, "η == 0.0005 ~ phantom(',') ~ s[z] == 0.6",
                                   ifelse(eta == 0.005 & sz == 0.1, "η == 0.005 ~ phantom(',') ~ s[z] == 0.1",
                                          "η == 0.005 ~ phantom(',') ~ s[z] == 0.6")))) %>%
  ggplot() + geom_point(aes(x = nu_tar, y = value, color = index), size = 0.3) + labs(color = "Index") +
  geom_line(aes(x = nu_tar, y = value, color = index), size = 0.3) + theme(legend.position = "top") +
  scale_color_manual(values = c("black", hue_pal()(3))) +
  scale_x_continuous(breaks = c(0.0015, 0.0025, 0.0035)) + xlab(expression(bold(paste({ν[target]})))) +
  facet_wrap(~ condition, labeller = label_parsed, nrow = 2, scales = "free_y") + ggtitle("Race model") -> p7_vrm
p7_vrm

p7_vrm <- p7_vrm + facetted_pos_scales(y = position_scales) + 
  guides(color = F) +
  theme(strip.text = element_text(size = 4.2, face = "bold", color = "black"),
        plot.margin = margin(5, 5, 5, 5), 
        plot.title = element_text(size = 5), 
...
# m-ratio (BE)
sdt_vrm %>%
  pivot_longer(cols = c("mratio_be", "mratio_rt", "mratio_logit_be"), 
               names_to = "index", values_to = "value") %>%
  mutate(index = fct_recode(index, "m-ratio confidence" = "mratio_be", "m-ratio RT" = "mratio_rt", "m-ratio logit" = "mratio_logit_be"),
  ...
  scale_x_continuous(breaks = c(0.0015, 0.0025, 0.0035)) + xlab(expression(bold(paste({ν[target]})))) +
  facet_wrap(~ condition, labeller = label_parsed, nrow = 2) +
  coord_cartesian(ylim = c(-0.3, 1.1)) + scale_y_continuous(breaks = seq(-0.2, 1, 0.4)) + ggtitle("Race model") -> p8_vrm
p8_vrm
p8_vrm <- p8_vrm + guides(color = F) +
  theme(strip.text = element_text(size = 4.2, face = "bold", color = "black"),
        plot.margin = margin(5, 5, 5, 5), 
        plot.title = element_text(size = 5), 
        axis.title.x = element_text(size = 5),
...
sdt_vrm %>%
  pivot_longer(cols = c("mratio_post", "mratio_rt", "mratio_logit_post"), 
               names_to = "index", values_to = "value") %>%
  mutate(index = fct_recode(index, "m-ratio confidence" = "mratio_post", "m-ratio RT" = "mratio_rt", "m-ratio logit" = "mratio_logit"),
         index = fct_relevel(index, "m-ratio confidence", "m-ratio RT", "m-ratio logit")) %>%
  mutate(condition = ifelse(eta == 5e-04 & sz == 0.1, "η == 0.0005 ~ phantom(',') ~ s[z] == 0.1",
                            ifelse(eta == 5e-04 & sz == 0.6, "η == 0.0005 ~ phantom(',') ~ s[z] == 0.6",
                                   ifelse(eta == 0.005 & sz == 0.1, "η == 0.005 ~ phantom(',') ~ s[z] == 0.1",
  ...
  geom_hline(yintercept = 1, linetype = "dashed") + theme(legend.position = "top") +
  scale_x_continuous(breaks = c(0.0015, 0.0025, 0.0035)) + xlab(expression(bold(paste({ν[target]})))) +
  facet_wrap(~ condition, labeller = label_parsed, nrow = 2) +
  coord_cartesian(ylim = c(-0.3, 1.1)) + scale_y_continuous(breaks = seq(-0.2, 1, 0.4)) + ggtitle("Race model") -> p9_vrm
p9_vrm
p9_vrm <- p9_vrm + guides(color = F) +
  theme(strip.text = element_text(size = 4.2, face = "bold", color = "black"),
        plot.margin = margin(5, 5, 5, 5), 
        plot.title = element_text(size = 5), 
        axis.title.x = element_text(size = 5),
...
# m-ratio boost by rt (BE)
sdt_vrm %>%
  pivot_longer(cols = c("be_logit_to_be"), 
               names_to = "index", values_to = "value") %>%
  mutate(condition = ifelse(eta == 5e-04 & sz == 0.1, "η == 0.0005 ~ phantom(',') ~ s[z] == 0.1",
                            ifelse(eta == 5e-04 & sz == 0.6, "η == 0.0005 ~ phantom(',') ~ s[z] == 0.6",
                                   ifelse(eta == 0.005 & sz == 0.1, "η == 0.005 ~ phantom(',') ~ s[z] == 0.1",
  ...
  ylab(expression(paste(meta, "-", d, "'"[confidence+RT]/meta, "-", d, "'"[confidence]))) +
  scale_x_continuous(breaks = c(0.0015, 0.0025, 0.0035)) + xlab(expression(bold(paste({ν[target]})))) +
  facet_wrap(~ condition, labeller = label_parsed, nrow = 2) +
  coord_cartesian(ylim = c(0.95, 1.8)) + scale_y_continuous(breaks = seq(1, 1.8, 0.2)) + ggtitle("Race model") -> p10_vrm
p10_vrm
p10_vrm <- p10_vrm + guides(color = F) +
  theme(strip.text = element_text(size = 4.2, face = "bold", color = "black"),
        plot.margin = margin(5, 5, 5, 5), 
        plot.title = element_text(size = 5), 
        axis.title.x = element_text(size = 5),
...
sdt_vrm %>%
  ...
  geom_hline(yintercept = 1, linetype = "dashed", size = 0.3) + theme(legend.position = "top") +
  scale_color_manual(values = c("#FFC425")) +
  ylab("Meta-d' boost due to RT") +
  scale_x_continuous(breaks = c(0.0015, 0.0025, 0.0035)) + xlab(expression(bold(paste({ν[target]})))) +
  facet_wrap(~ condition, labeller = label_parsed, nrow = 2) +
  coord_cartesian(ylim = c(0.95, 1.65)) + scale_y_continuous(breaks = seq(1, 1.6, 0.2)) + ggtitle("Race model") -> p11_vrm
p11_vrm
p11_vrm <- p11_vrm + guides(color = F) +
  theme(strip.text = element_text(size = 4.2, face = "bold", color = "black"),
        plot.margin = margin(5, 5, 5, 5), 
        plot.title = element_text(size = 5), 
        axis.title.x = element_text(size = 5),
...
# correctness x rt x be confidence
simdat_vrm$rt_bin <- cut(simdat_vrm$rt, breaks = quantile(simdat_vrm$rt, probs = seq(0, 1, by = 0.25)), include.lowest = TRUE)
levels(simdat_vrm$rt_bin) <- c("q1", "q2", "q3", "q4")
simdat_vrm %>%
  group_by(choice, nu_tar, eta, sz, rt_bin) %>%
  summarise(mean_conf = mean(mvn_response), n = n()) %>%
  ggplot() +
  ...
  facet_wrap(. ~ nu_tar + eta + sz, ncol = 4,
             labeller = labeller(nu_tar = as_labeller(c("0.0015" = "ν[target] == 0.0015", "0.0025" = "ν[target] == 0.0025", 
                                 ...
                                 eta = as_labeller(c("0.0005" = "η == 0.0005", "0.005" = "η == 0.005"), label_parsed),
                                 sz = as_labeller(c("0.1" = "s[z] == 0.1", "0.6" = "s[z] == 0.6"), label_parsed))) +
  xlab("RT quantile") + ylab("Mean confidence") + scale_color_npg() + 
  theme(axis.title = element_text(size = 7), legend.position = "top", plot.title = element_text(size = 7),
        strip.text = element_text(size = 7, margin = margin(-0.5, -0.5, -0.5, -0.5, "pt"))) + ggtitle("Race model") -> p12_vrm
p12_vrm
p12_vrm <- p12_vrm + theme(legend.text = element_text(size = 7),
...
simdat_vrm %>%
  ...
  ggplot() + 
  geom_line(aes(x = rt_bin, y = mean_conf, color = choice, group = choice)) +
  geom_point(aes(x = rt_bin, y = mean_conf, color = choice)) +
  facet_wrap(. ~ nu_tar + eta + sz, ncol = 4,
             labeller = labeller(nu_tar = as_labeller(c("0.0015" = "ν[target] == 0.0015", "0.0025" = "ν[target] == 0.0025", 
                                                           "0.0035" = "ν[target] == 0.0035"), label_parsed),
                                 eta = as_labeller(c("0.0005" = "η == 0.0005", "0.005" = "η == 0.005"), label_parsed),
                                 sz = as_labeller(c("0.1" = "s[z] == 0.1", "0.6" = "s[z] == 0.6"), label_parsed))) +
  xlab("RT quantile") + ylab("Mean post confidence") + scale_color_npg() +
  theme(axis.title = element_text(size = 7), legend.position = "top", plot.title = element_text(size = 7),
        strip.text = element_text(size = 7, margin = margin(-0.5, -0.5, -0.5, -0.5, "pt"))) + ggtitle("Race model") -> p13_vrm
p13_vrm
p13_vrm <- p13_vrm + guides(color = F)
sjPlot::save_plot("p13_vrm.png", p13_vrm, width = 15, height = 12, dpi = 300)

# rt x be conf x accuracy
simdat_vrm$rt_bin <- cut(simdat_vrm$rt, breaks = quantile(simdat_vrm$rt, probs = seq(0, 1, 0.25)), include.lowest = TRUE)
levels(simdat_vrm$rt_bin) <- c("q1", "q2", "q3", "q4")
simdat_vrm$conf_bin <- cut(simdat_vrm$mvn_response, breaks = quantile(simdat_vrm$mvn_response, probs = seq(0, 1, 0.25)), include.lowest = TRUE)
levels(simdat_vrm$conf_bin) <- c("q1", "q2", "q3", "q4")
simdat_vrm %>%
  ...
  theme(axis.title = element_text(size = 7), legend.position = "top", plot.title = element_text(size = 7),
        ...
        plot.margin = unit(c(1, 1, 1, 1), "pt")) + ggtitle("Race model") -> p14_vrm
p14_vrm
p14_vrm <- p14_vrm + guides(color = F)
sjPlot::save_plot("p14_vrm.png", p14_vrm, width = 15, height = 15, dpi = 300)

# rt x be conf x accuracy (binned for each condition)
simdat_vrm_bin <- c()
for (i in nu_tar) {
  for (j in eta) {
    for (k in sz) {
      subdat_vrm <- filter(simdat_vrm, nu_tar == i, eta == j, sz == k)
      subdat_vrm$rt_bin <- cut(subdat_vrm$rt, breaks = quantile(subdat_vrm$rt, probs = seq(0, 1, 0.25)), include.lowest = TRUE)
      levels(subdat_vrm$rt_bin) <- c("q1", "q2", "q3", "q4")
      subdat_vrm$conf_bin <- cut(subdat_vrm$mvn_response, breaks = quantile(subdat_vrm$mvn_response, probs = seq(0, 1, 0.25)), include.lowest = TRUE)
      levels(subdat_vrm$conf_bin) <- c("q1", "q2", "q3", "q4")
      simdat_vrm_bin <- rbind(simdat_vrm_bin, subdat_vrm)
    }
  }
}
...
simdat_vrm_bin %>%
  ...
  theme(axis.title = element_text(size = 7), legend.position = "top", plot.title = element_text(size = 7),
        strip.text = element_text(size = 7, margin = margin(-0.5, -0.5, -0.5, -0.5, "pt")),
        plot.margin = unit(c(1, 1, 1, 1), "pt")) + ggtitle("Race model") -> p15_vrm
p15_vrm
p15_vrm <- p15_vrm + guides(color = F)
sjPlot::save_plot("p15_vrm.png", p15_vrm, width = 15, height = 15, dpi = 300)
...
simdat_vrm_bin %>%
  ...
  theme(axis.title = element_text(size = 7), legend.position = "top", plot.title = element_text(size = 7),
        strip.text = element_text(size = 7, margin = margin(-0.5, -0.5, -0.5, -0.5, "pt")),
        plot.margin = unit(c(1, 1, 1, 1), "pt")) + ggtitle("Race model") 
...
# rt x post conf x accuracy
simdat_vrm$rt_bin <- cut(simdat_vrm$rt, breaks = quantile(simdat_vrm$rt, probs = seq(0, 1, 0.25)), include.lowest = TRUE)
levels(simdat_vrm$rt_bin) <- c("q1", "q2", "q3", "q4")
simdat_vrm$conf_bin <- cut(simdat_vrm$post_conf, breaks = quantile(simdat_vrm$post_conf, probs = seq(0, 1, 0.25)), include.lowest = TRUE)
levels(simdat_vrm$conf_bin) <- c("q1", "q2", "q3", "q4")
simdat_vrm %>%
  group_by(nu_tar, eta, sz, rt_bin, conf_bin) %>%
  summarise(accuracy = mean(correct)) %>%
  ...
  facet_wrap(. ~ nu_tar + eta + sz, ncol = 4,
             labeller = labeller(nu_tar = as_labeller(c("0.0015" = "ν[target] == 0.0015", "0.0025" = "ν[target] == 0.0025", 
                                 ...
                                 sz = as_labeller(c("0.1" = "s[z] == 0.1", "0.6" = "s[z] == 0.6"), label_parsed))) +
  theme(legend.position = "top") + labs(color = "RT bin") +
  xlab("Post confidence bin") + ylab("Accuracy") + coord_cartesian(ylim = c(0.5, 1)) +
  theme(axis.title = element_text(size = 7), legend.position = "top", plot.title = element_text(size = 7),
        strip.text = element_text(size = 7, margin = margin(-0.5, -0.5, -0.5, -0.5, "pt")),
        plot.margin = unit(c(1, 1, 1, 1), "pt")) + ggtitle("Race model") -> p16_vrm
p16_vrm
p16_vrm <- p16_vrm + guides(color = F)
sjPlot::save_plot("p16_vrm.png", p16_vrm, width = 15, height = 15, dpi = 300)

# rt x post conf x accuracy (binned for each condition)
simdat_vrm_bin <- c()
for (i in nu_tar) {
  for (j in eta) {
    for (k in sz) {
      ...
      subdat_vrm$rt_bin <- cut(subdat_vrm$rt, breaks = quantile(subdat_vrm$rt, probs = seq(0, 1, 0.25)), include.lowest = TRUE)
      levels(subdat_vrm$rt_bin) <- c("q1", "q2", "q3", "q4")
      subdat_vrm$conf_bin <- cut(subdat_vrm$post_conf, breaks = quantile(subdat_vrm$post_conf, probs = seq(0, 1, 0.25)), include.lowest = TRUE)
      levels(subdat_vrm$conf_bin) <- c("q1", "q2", "q3", "q4")
      simdat_vrm_bin <- rbind(simdat_vrm_bin, subdat_vrm)
    }
  }
...
simdat_vrm %>%
  ...
  theme(axis.title = element_text(size = 7), legend.position = "top", plot.title = element_text(size = 7),
        strip.text = element_text(size = 7, margin = margin(-0.5, -0.5, -0.5, -0.5, "pt")),
        plot.margin = unit(c(1, 1, 1, 1), "pt")) + ggtitle("Race model") -> p17_vrm
p17_vrm
p17_vrm <- p17_vrm + guides(color = F)
sjPlot::save_plot("p17_vrm.png", p17_vrm, width = 15, height = 15, dpi = 300)

# type-1 roc
roc_1_vrm %>% filter(variable != "post" & variable != "logit_post") %>%
  ...
  facet_wrap(. ~ nu_tar + eta + sz, ncol = 4,
             labeller = labeller(nu_tar = as_labeller(c("0.0015" = "ν[target] == 0.0015", "0.0025" = "ν[target] == 0.0025", 
                                 ...
                                 eta = as_labeller(c("0.0005" = "η == 0.0005", "0.005" = "η == 0.005"), label_parsed),
                                 sz = as_labeller(c("0.1" = "s[z] == 0.1", "0.6" = "s[z] == 0.6"), label_parsed))) +
  xlab("p(response = S2|stimulus = S1)") + ylab("p(response = S2|stimulus = S2)") + labs(color = "Variable") + 
  scale_x_continuous(breaks = c(0, 0.5, 1)) + scale_y_continuous(breaks = c(0, 0.5, 1)) +
  theme(axis.title = element_text(size = 7), legend.position = "top", plot.title = element_text(size = 7),
        strip.text = element_text(size = 7, margin = margin(-0.5, -0.5, -0.5, -0.5, "pt"))) + ggtitle("Race model") -> p18_vrm
p18_vrm
p18_vrm <- p18_vrm + 
  theme(axis.text.x = element_text(angle = 25),
        strip.text = element_text(size = 6.5, face = "bold"),
        plot.margin = margin(5, 5, 5, 5), 
        plot.title = element_text(size = 8)) + guides(color = F)

# type-2 roc
roc_2_vrm %>% filter(variable != "post" & variable != "logit_post") %>%
  mutate(variable = fct_recode(variable, "confidence" = "be", "RT" = "RT", "logit" = "logit_be"),
         variable = fct_relevel(variable, "confidence", "RT", "logit")) %>%
  ...
  facet_wrap(. ~ nu_tar + eta + sz, ncol = 4,
             labeller = labeller(nu_tar = as_labeller(c("0.0015" = "ν[target] == 0.0015", "0.0025" = "ν[target] == 0.0025", 
                                 ...
                                 eta = as_labeller(c("0.0005" = "η == 0.0005", "0.005" = "η == 0.005"), label_parsed),
                                 sz = as_labeller(c("0.1" = "s[z] == 0.1", "0.6" = "s[z] == 0.6"), label_parsed))) +
  xlab("p(secondary variable = high|response = incorrect)") + ylab("p(secondary variable = high|response = correct)") + labs(color = "Variable") + 
  scale_x_continuous(breaks = c(0, 0.5, 1)) + scale_y_continuous(breaks = c(0, 0.5, 1)) +
  theme(axis.title = element_text(size = 7), legend.position = "top", plot.title = element_text(size = 7),
        strip.text = element_text(size = 7, margin = margin(-0.5, -0.5, -0.5, -0.5, "pt"))) + ggtitle("Race model") -> p19_vrm
p19_vrm
p19_vrm <- p19_vrm + 
  theme(axis.text.x = element_text(angle = 25),
        strip.text = element_text(size = 6.5, face = "bold"),
...
simdat_vrm %>%
  ...
  scale_x_continuous(breaks = c(0.0015, 0.0025, 0.0035)) + xlab(expression(bold(paste({ν[target]})))) +
  facet_wrap(~ condition, labeller = label_parsed, nrow = 2) +
  ylab("Mean early confidence") + scale_color_npg() + ggtitle("Race model") -> p20_vrm
p20_vrm
p20_vrm <- p20_vrm + guides(color = F) + theme(axis.text.x  = element_text(angle = 20), plot.title = element_text(size = 7))
