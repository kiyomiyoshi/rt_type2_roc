...
for (j in 1:length(script)) {
  source(script[j]) 
  cor_df_mdiff$exp <- dataset[j] 
  data <- rbind(data, cor_df_mdiff)
}  
...
df_forest_relabeled <- df_forest %>%
  mutate(pair = factor(pair,
                       levels = c("dp-mdp_conf", "dp-mdp_rt", "dp-mdp_logit",
                                  "mdp_conf-mdp_rt", "mdp_conf-mdp_logit", "mdp_rt-mdp_logit"),
                       labels = c(
                         "d*\"'\"*\" vs. \"*meta*\"-\"*d*\"'\"[confidence]",
                         "d*\"'\"*\" vs. \"*meta*\"-\"*d*\"'\"[RT]",
                         "d*\"'\"*\" vs. \"*meta*\"-\"*d*\"'\"[confidence+RT]",
                         "meta*\"-\"*d*\"'\"[confidence]*\" vs. \"*meta*\"-\"*d*\"'\"[RT]",
                         "meta*\"-\"*d*\"'\"[confidence]*\" vs. \"*meta*\"-\"*d*\"'\"[confidence+RT]",
                         "meta*\"-\"*d*\"'\"[RT]*\" vs. \"*meta*\"-\"*d*\"'\"[confidence+RT]"
                       )))
...
ggplot(df_forest_relabeled, aes(y = exp, x = r)) +
  ...
  theme_bw() +
  coord_cartesian(xlim = c(-0.53, 1.03)) +
  theme(axis.text.y = element_text(size = 8),
        axis.title.x = element_text(size = 10),
        strip.text = element_text(size = 8)) -> g
g
ggsave("figure_2.png", g, height = 4.72 * 1.1, width = 4.72 * 1.55)