...
dat <- subset(dat, dat$Difficulty == "2") # time exposure of the stimuli (1 = 850ms, 2 = 700ms, 3 = 550ms). Difficulty 1 and 3 in Study 1; difficulty 2 in Study 2.
dat <- subset(dat, dat$Confidence >= 0.5)
dat <- subset(dat, dat$Subj_idx != 76) # weird RT_dec
...
df <- foreach(i = 1:length(unique(dat$Subj_idx)), .combine = "rbind", .packages = c("dplyr", "metaSDT")) %dopar% {
  ...
  # confidence
  nR_S1 <- c(sum(d$Stimulus == "1" & d$Response == "1" & d$Confidence >= 0.85),
             sum(d$Stimulus == "1" & d$Response == "1" & d$Confidence <= 0.80 & d$Confidence >= 0.70),
             sum(d$Stimulus == "1" & d$Response == "1" & d$Confidence <= 0.65),
             sum(d$Stimulus == "1" & d$Response == "2" & d$Confidence <= 0.65),
             sum(d$Stimulus == "1" & d$Response == "2" & d$Confidence <= 0.80 & d$Confidence >= 0.70),
             sum(d$Stimulus == "1" & d$Response == "2" & d$Confidence >= 0.85))
  
  nR_S2 <- c(sum(d$Stimulus == "2" & d$Response == "1" & d$Confidence >= 0.85),
             sum(d$Stimulus == "2" & d$Response == "1" & d$Confidence <= 0.80 & d$Confidence >= 0.70),
             sum(d$Stimulus == "2" & d$Response == "1" & d$Confidence <= 0.65),
             sum(d$Stimulus == "2" & d$Response == "2" & d$Confidence <= 0.65),
             sum(d$Stimulus == "2" & d$Response == "2" & d$Confidence <= 0.80 & d$Confidence >= 0.70),
             sum(d$Stimulus == "2" & d$Response == "2" & d$Confidence >= 0.85))
  ...
  d$logit <- predict(m)
  q <- quantile(d$logit, probs = seq(0, 1, by = 1/3), na.rm = TRUE)
  d$logit_bin <- cut(d$logit, breaks = q, labels = FALSE, include.lowest = TRUE)
  
  nR_S1 <- c(sum(d$Stimulus == "1" & d$Response == "1" & d$logit_bin == 3,  na.rm = TRUE),
  ...
  nR_S2 <- c(sum(d$Stimulus == "2" & d$Response == "1" & d$logit_bin == 3,  na.rm = TRUE),
